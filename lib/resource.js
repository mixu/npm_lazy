...
Resource.prototype._fetchTask = function(onDone) {
  var match;
  var self = this;
  var url = coreUrl.parse(this.url302),
      opts = {
        method: 'GET',
        path: url.path,
        port: url.port,
  ...
  if (proxyConfig && proxyConfig.hostname) {
    opts.hostname = proxyConfig.hostname;
    opts.port = proxyConfig.port;
    // full path with url
    opts.path = this.url;
    opts.headers = {
      Host: url.hostname,
    ...
    isHttps = (proxyConfig.protocol == 'https:');
  }
  ...
  req = (isHttps ? https : http).get(opts, function(res){
    if (res.statusCode != 200) {
      if (302 == res.statusCode) {
          self.url302 = res.headers["location"];
          return onDone(new Error('Request from [' + self.url + '] redirect to [' + self.url302 + ']'));
      } else {
