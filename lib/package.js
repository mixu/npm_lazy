...
Package.proxy = function(req, res) {
  var uri = remoteUrl + req.url.substr(1); // because we ask that there is a trailing / on the registry url
  console.log('Querying the registry (not cached):', uri);
  req.pipe((remoteIsHttps ? https : http).request(uri, function(pres) {
    pres.pipe(res);
  })).once('error', function(e) {
    console.log('Ignoring query error (not cached):', e);
    res.statusCode = 500;
    res.end('{}');
  });
};
